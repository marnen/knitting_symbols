...
**N.B.!** The 90 dpi figure was used for historical reasons, but SVG 2, Inkscape, and all browser implementations [now assume 96 dpi, not 90](https://inkscape.org/learn/faq/#dpi_change), so we're changing to 96 DPI as of release 0.6.0. We are keeping the pixels the same, which won't affect screen display, but if you're using the SVG files in the release package for print, be aware that you may have to scale by 16/15 (108.3333%) to maintain the size of the original reference pattern (the PNG files should maintain their original resolution of 90 dpi).
